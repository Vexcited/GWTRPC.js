...
export const handle_classes = (bundle: Program, classes: ReturnType<typeof extract_classes>) => {
  let RemoteServiceProxy$ServiceHelper: types.FunctionDeclaration | undefined;

  const proxy_type_ids = classes.proxies.map((proxy) => proxy.type_id);
  ...
  traverse(bundle, {
    ...
    CallExpression({ node, scope }) {
      ...
      const arg0_ok = is.literal(arg0) && typeof arg0.value === "number" && (
        proxy_type_ids.includes(arg0.value) || arg0.value === classes.RemoteServiceProxy$ServiceHelper.type_id
      );
      ...
      if (!binding || !is.functionDeclaration(binding.path.node))
        return;

      // from there, we can intercept the `RemoteServiceProxy$ServiceHelper` node
      if (type_id === classes.RemoteServiceProxy$ServiceHelper.type_id) {
        RemoteServiceProxy$ServiceHelper = binding.path.node;
        return;
      }

      // get GWT class from known classes.
      const gwt_class = classes.proxies.find((proxy) => proxy.type_id === type_id);
      if (!gwt_class) return;
      ...
      if (!is.expressionStatement(statement) || !is.callExpression(statement.expression)) return;
      ...
      if (!path || !hash) {
        console.warn(`[${gwt_class.class_name}]: 'path' is null! skipping...`);
        return;
      }
